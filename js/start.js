	function start(error,data){
		...
		/** Set up scales **/
  		...
  		xEmpFTI.domain([0, d3.max(data_sort, function(d) { return parseFloat(d.EmpFTI); })]);
    ...
    chart.append("rect")
      ...
      .attr("visibility","hidden");
    	...
    	bar.append("rect")
    		.attr("class",function(d){return "EmpFTI"+" "+d.Country.replace(/\s+/g, '');  }) //remove space fro name to facilitate selection
    		.attr("x",2*width)
    		.attr("y",function(d, i) { return  i * barHeight ; })
    		.attr("width", function(d) { return xEmpFTI(d.EmpFTI); })
      		.attr("height", barHeight -1)
      		.attr("fill",function(d){
      			if(d.Country=="OECD"){
      				return "#0B1E2D"
      			}else{
      				return colorScale(assignSector("EmpFTI"));
      			}
      		})
      		.attr("opacity",function(d){
      			return opacityIndicator(d.EmpFTI,d.Country,"EmpFTI");
      		});
